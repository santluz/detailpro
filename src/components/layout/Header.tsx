...
export function Header() {
  const { sidebarOpen, darkMode, toggleDarkMode, globalSearch, setGlobalSearch, toggleSidebar } = useAppStore();
  const { user } = useAuth();
  ...
  return (
    <header className={cn(
      ...
      <button onClick={toggleSidebar} className="lg:hidden btn-ghost">
      ...
      <div className="flex items-center gap-2 ml-auto">
        <button onClick={toggleDarkMode} className="btn-ghost" title={darkMode ? 'Modo claro' : 'Modo escuro'}>
