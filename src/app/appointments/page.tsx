...
function AppointmentModal({ clients, services, employees, appointment, onClose, onSave }: {
  ...
  const [form, setForm] = useState({
    ...
    date: appointment?.date
      ? (appointment.date instanceof Date ? appointment.date : new Date(appointment.date as unknown as string)).toISOString().split('T')[0]
  ...
  useEffect(() => {
    if (form.clientId && companyId) {
      vehiclesService.getByClient(companyId, form.clientId)
        .then(v => setVehicles(v as unknown as Vehicle[]))
        .catch(() => setVehicles([]));
    } else {
      setVehicles([]);
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.clientId) return toast.error('Selecione o cliente');
    if (!form.serviceId) return toast.error('Selecione o serviço');
    if (!form.date || !form.time) return toast.error('Informe data e horário');
    ...
    try {
      ...
      const data = {
        ...
        vehicleModel: vehicle ? `${vehicle.brand} ${vehicle.model}` : '',
    ...
    <div className="modal-overlay" onClick={onClose}>
      <div className="modal w-full max-w-lg" onClick={e => e.stopPropagation()}>
        <div className="flex items-center justify-between p-5 border-b border-gray-100 dark:border-gray-800">
          <h2 className="font-semibold text-gray-900 dark:text-white">{appointment ? 'Editar Agendamento' : 'Novo Agendamento'}</h2>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="col-span-2">
              ...
              <select className="input" value={form.vehicleId} onChange={set('vehicleId')} disabled={!form.clientId}>
                <option value="">Sem veículo / Selecione</option>
                {vehicles.map(v => <option key={v.id} value={v.id}>{v.brand} {v.model} - {v.plate}</option>)}
              </select>
              {form.clientId && vehicles.length === 0 && (
                <p className="text-xs text-gray-400 mt-1">Nenhum veículo cadastrado para este cliente</p>
              )}
            </div>
            <div className="col-span-2">
...
export default function AppointmentsPage() {
  ...
  const calendarEvents = appointments.map(a => ({
    ...
    date: a.date instanceof Date
      ...
      : new Date(a.date as unknown as string).toISOString().split('T')[0],
    backgroundColor: STATUS_COLORS[a.status]?.includes('blue') ? '#3B82F6'
      : STATUS_COLORS[a.status]?.includes('yellow') ? '#F59E0B'
      : STATUS_COLORS[a.status]?.includes('green') ? '#10B981' : '#EF4444',
    ...
    <div className="space-y-6">
      ...
      <div className="card p-5">
        <FullCalendar
          ...
          eventClick={(info) => setModal({ open: true, appointment: info.event.extendedProps.appointment as Appointment })}
          height="auto"
          dayMaxEvents={3}
        />
      </div>
